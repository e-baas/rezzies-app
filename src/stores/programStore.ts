...
interface ProgramState {
  ...
  joinProgram: (inviteCode: string) => Promise<{ programId?: string; error?: string }>;
...
export const useProgramStore = create<ProgramState>((set) => ({
  ...
  createProgram: async (data) => {
    ...
    const { data: program, error: programError } = await supabase
      .from('programs').insert({
        name: data.name, description: data.description, sponsor_id: user.id,
        ...
        prize_structure: 'PLACEMENT', prize_pool_amount: 0, prize_config: {},
      }).select().single();
    ...
    await supabase.from('participants').insert({
      ...
      total_points: 0, total_habits: 0, current_streak: 0, health_data_consent: false,
    });
    ...
    if (periodInserts.length > 0) {
      ...
      if (periods && data.monthlyBonuses.length > 0) {
        const bonusInserts = data.monthlyBonuses.map((b, i) => ({
          program_id: program.id, period_id: periods[Math.min(i, periods.length - 1)].id,
  ...
  joinProgram: async (inviteCode) => {
    ...
    if (existing) return { programId: program.id };
    await supabase.from('participants').insert({
